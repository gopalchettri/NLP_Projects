...
class WordVecCnn(object):
    ...
    def load_model(self, model_dir_path):
        json = open(self.get_architecture_file_path(
            model_dir_path), 'r').read()
    ...
    def create_model(self):
        ...
        self.model.add(Embedding(input_dim=self.vocab_size,
                       input_length=self.max_len, output_dim=embedding_size))
        self.model.add(SpatialDropout1D(0.2))
        self.model.add(Conv1D(filters=256, kernel_size=5,
    ...
    def predict(self, sentence):
        ...
        wid = [self.word2idx[token] if token in self.word2idx else len(
            self.word2idx) for token in tokens]
        xs.append(wid)
    ...
    def fit(self, text_data_model, text_label_pairs, model_dir_path, batch_size=None, epochs=None,
        ...
        Y = utils.to_categorical(ys, len(self.labels))
        ...
        np.save(model_dir_path + '/' + WordVecCnn.model_name +
...
class WordVecMultiChannelCnn(object):
    ...
    def fit(self, text_data_model, text_label_pairs, model_dir_path,
        ...
        np.save(model_dir_path + '/' + WordVecMultiChannelCnn.model_name +
                '-history.npy', history.history)
