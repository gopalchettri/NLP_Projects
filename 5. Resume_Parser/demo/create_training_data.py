...
class AnnotatorGui(Frame):
    ...
    def build_line(self, table_content, line_index, line):
        ...
        line_index_label = Label(
            self, width=10, height=1, text=str(line_index))
...
def command_line_annotate(training_data_dir_path, index, file_path, file_content):
    with open(os.path.join(training_data_dir_path, str(index) + '.txt'), 'wt', encoding='utf8') as f:
        for line_index, line in enumerate(file_content):
            ...
            data_type = input('Type for line #' + str(line_index) +
                              ' (options: 0=header 1=meta 2=content):')
            label = input('Label for line #' + str(line_index) +
                          ' (options: 0=experience 1=knowledge 2=education 3=project 4=others 5=name 6=email 7=gender 8=mobile 9=objective 10=sex 11=ethnicity')
            data_type = int(data_type)
            label = int(label)
            f.write(line_types[data_type] + '\t' +
                    line_labels[label] + '\t' + line)
            f.write('\n')
...
def gui_annotate(training_data_dir_path, index, file_path, file_content):
    root = Tk()
    table_content = [[line, guess_line_type(
        line), guess_line_label(line)] for line in file_content]
    gui = AnnotatorGui(root, table_content)

    def callback():
        root.destroy()
        output_file_path = os.path.join(
            training_data_dir_path, str(index) + '.txt')
        ...
        with open(output_file_path, 'wt', encoding='utf8') as f:
            for line in table_content:
                ...
                f.write(line_types[data_type] + '\t' +
                        line_labels[label] + '\t' + line_content)
                f.write('\n')
